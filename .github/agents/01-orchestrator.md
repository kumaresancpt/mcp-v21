...
- NEVER commit to `main` — 13-github-agent creates versioned feature branches from user-selected base-branch
...
- 13-github-agent creates new versioned branches in STEP 7a (during commit-files operation)
...
## Phase 10 — GitHub PR
- Base branch selected: feature/<work-item-id>-v1  (e.g. feature/PROJ-42-v1, from STEP 1)
- New versioned branch created: feature/<work-item-id>-v2 (auto-versioned by 13-github-agent in STEP 7a)
- Commit: feat(<work-item-id>): full implementation
- PR: feature/<work-item-id>-v2 → main
- PR title: feat(<work-item-id>): <story-title>
...
### 7a — Delegate to `13-github-agent`: commit-files (PHASE 1 — Review Files)

Delegate to `13-github-agent` with operation `commit-files`:
- `owner`, `repo`
- `base-branch`: `<base-branch>` — the branch pulled by `03-repo-reader` (e.g. feature/SCRUM-18-v1)
- `commit-message`: `feat(<work-item-id>): <story-title>`
- `local-workspace-root`: `<local-workspace-root>`
- `project-root`: `<project-root>`

The `13-github-agent` will:
1. Create a new versioned branch FROM base-branch (e.g., feature/SCRUM-18-v2 from feature/SCRUM-18-v1)
2. Use `git diff` to detect only changed and new files from base-branch
3. Filter out excluded files (.gstack/, output/, node_modules/, etc.)
4. **RETURN the filtered file list** (do NOT commit yet)

**RECEIVE FILE LIST FROM 13-github-agent, then:**

Show the user:
> "📋 **Files ready to commit to `<new-versioned-branch>`:**
> 
> <count> files will be committed:
> 
> **Frontend Files:**
> - <file list>
> 
...
**If user replies `no`:**
- Show: "❌ Commit cancelled. No files were pushed to GitHub."
- ABORT STEP 7 entirely — do NOT call 7b (create-pr)
- Return to STEP 6 for user to review and re-run

---

### 7a — PHASE 2 — Execute Commit (after user approval)

Send approval signal to `13-github-agent` to proceed with commit:
- `approved = true`
- `files-list`: <the approved files>

The `13-github-agent` will:
1. Read each file from local disk
2. Push all files to the new versioned branch in one commit
3. Return confirmation: new versioned branch name + list of committed files
...
- `new-versioned-branch`: the branch name returned from 7a PHASE 2 (e.g., feature/SCRUM-18-v2)
- `pr-body`: contents of `output/changelog.md`

The PR will be created:
- **From**: `<new-versioned-branch>` (e.g., feature/SCRUM-18-v2)
- **To**: `main`
