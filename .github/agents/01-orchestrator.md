...
### 3c-GEN — After `06-backend-writer` completes: Generate Schema Plan (`07-database-agent` generate-schema-plan operation)

Invoke `07-database-agent` with operation `generate-schema-plan`.
Pass `owner`, `repo`, `work-item-id`, `project-root`.

**Expected:** Agent will pause and ask for:
- Database name
- PostgreSQL username  
- PostgreSQL password

Wait for user to provide all three values, then let agent continue.

**Expected output:** `.gstack/schema-plan-generated.md` is written to disk.

Wait for agent to confirm: "✅ Schema plan generated and saved to .gstack/schema-plan-generated.md"

---

### 3c-APPROVE — Schema Plan Approval Gate (ORCHESTRATOR APPROVAL STEP — NEW)

**THIS IS A HARD STOP — ORCHESTRATOR LEVEL APPROVAL.**

1. Read `.gstack/schema-plan-generated.md` from disk
2. Display the schema plan to user with clear formatting
3. Ask user: "**Approve this database schema plan? (yes/no/show-details)**"
4. Wait for response:
   - **yes** → Continue to 3c-EXEC
   - **no** → Report "❌ Schema plan rejected by user. Database setup cancelled." → HALT pipeline, ask user if they want to modify and retry
   - **show-details** → Display full plan details again, then re-ask approval question

This is a CRITICAL user approval gate. Never skip this step.

---

### 3c-EXEC — After user approval: Run Migration (`07-database-agent` run-migration operation)

Invoke `07-database-agent` with operation `run-migration`.
Pass `owner`, `repo`, `work-item-id`, `project-root`, `approval-status="approved"`.

**Expected:** Agent will verify approval-status="approved", then run the migration.

Wait for `07-database-agent` to confirm "✅ Database migration completed successfully. Results written to output/ folder."

---

### 3d — After `07-database-agent` (run-migration) completes: Run `08-integration-patcher`
...
## STEP 6b — Generate Agent Execution Log (Human-Readable)

**Convert usage-log.jsonl to markdown format for transparency.**

**INSTRUCTIONS FOR RUNNING AGENT:**

1. **Read usage-log.jsonl** — Read `.gstack/usage-log.jsonl` line by line
   - Each line is a separate JSON object
   - Parse each line as valid JSON

2. **Extract fields from each log entry:**
   ```
   For each JSON object:
   - agent (string): Name of agent
   - phase (string): Phase category (Frontend, Backend, Database, etc.)
   - status (string): success | failed | skipped
   - start_time (ISO 8601): When agent started
   - end_time (ISO 8601): When agent ended
   - duration_ms (number): Duration in milliseconds
   - mcp_calls (number): Count of MCP calls made
   - input_tokens (number): Tokens used for input
   - output_tokens (number): Tokens used for output
   - [optional] files_written (number): Count of files
   - [optional] issues_found (number): Count of issues
   - [optional] tables_created (number): For database
   - [optional] tables_modified (number): For database
   - [optional] migration_name (string): Migration name
   - [optional] user_approval (string): Approval status
   - [optional] quality_gate (string): Sonar result
   - [optional] notes (string): Additional info
   ```

4. **Generate markdown content** with the following structure:

**STEP 6b Algorithm (AUTOMATIC CALCULATION):**

1. Read `.gstack/usage-log.jsonl` line by line
2. For each agent log entry:
   - Extract: ai_calls, input_tokens, output_tokens, model
   - **Auto-lookup model multiplier** from model-to-multiplier table
   - **Auto-calculate premium_requests** = (ai_calls × (input_tokens + output_tokens) × multiplier) ÷ 7,500
   - Format and display with calculated values
3. Sum all premium_requests for total
4. Calculate monthly estimates at various frequencies
5. Show warnings if exceeding quota

   ```markdown
   # Agent Execution Log — Work Item <work-item-id>
   
   Generated: <timestamp from file write>
   ...
   - ⏭️ Skipped: <count of status='skipped'>
   - Total duration: <sum all duration_ms, convert to MM:SS format>
   - Total MCP calls: <sum all mcp_calls>
   
   ---
   
   ## Agent Details
   
   <For each log entry, generate section:>
   ...
   **Status:** <✅ Success | ❌ Failed | ⏭️ Skipped>
   ...
   | End | <end_time ISO format> |
   | Duration | <duration_ms ms (convert to MM:SS)> |
   | MCP Calls | <mcp_calls> |
   | Input Tokens | <input_tokens> |
   | Output Tokens | <output_tokens> |
   
   <If phase='Frontend': - Files Written: <files_written>>
   <If phase='Backend': - API Routes: <files_written or count>>
   <If phase='Database': - Tables Created: <tables_created> | Modified: <tables_modified> | Migration: <migration_name> | Approved: <user_approval>>
   <If phase='Sonar': - Issues Found: <issues_found> | Quality Gate: <quality_gate>>
   <If phase='Integration': - Files Patched: <files_written>>
   <If notes is present: - **Notes:** <notes>>
   
   ---
   
   <End for-loop>
   
   ## Premium Requests Summary (GitHub Copilot Business Plan)
   
   **Calculation Method:** Automatic (no manual entry required)
   
   | Metric | Value |
   |--------|-------|
   | **Formula** | (AI calls × tokens × model_multiplier) ÷ 7,500 |
   | **Model Lookup** | Automatic from model field |
   | **Total Premium Requests This Run** | <sum all premium_requests from all agents> |
   | **Total AI Calls** | <sum all ai_calls> |
   | **Total Tokens** | <sum of (input_tokens + output_tokens)> |
   | **Estimated Monthly (10 runs)** | <total_premium_requests × 10> |
   | **Estimated Monthly (50 runs)** | <total_premium_requests × 50> |
   | **Standard Business Plan Limit** | 1,000 requests/month |
   
   ### Breakdown by Agent (Auto-Calculated)
   
   | Agent | AI Calls | Tokens | Model | Multiplier | Premium Requests |
   |-------|----------|--------|-------|-----------|------------------|
   <For each agent in log:>
   | <agent> | <ai_calls> | <input_tokens + output_tokens> | <model> | <auto-looked-up multiplier> | <auto-calculated premium_requests> |
   
   ---
   
   <if total_premium_requests × 10 > 900:>
   
   ⚠️ **BILLING WARNING:** At 10 runs/month frequency, you would use <total_premium_requests × 10> premium requests/month (over limit by <(total_premium_requests × 10) - 1000>)
   
   <end if>
   
   <if total_premium_requests × 50 > 1000:>
   
   ⚠️ **BILLING CRITICAL:** At 50 runs/month frequency, you would use <total_premium_requests × 50> premium requests/month (over limit by <(total_premium_requests × 50) - 1000>)
   - Consider: Reduce token input, use Haiku model, or upgrade plan
   ...
   ## Token Summary
   - Total Input Tokens: <sum all input_tokens>
   - Total Output Tokens: <sum all output_tokens>
   
   ## Next Steps
   1. Review this execution log for any failures or issues
   2. Check output/changelog.md for detailed implementation
   3. Review premium requests usage (see section above for billing impact)
   4. Run the app locally and verify functionality
   5. Create pull request from feature branch to main
   ```

4. **Write to output/agent-execution-log.md** with the formatted content above
...
- [ ] `07-database-agent` (generate-schema-plan) confirmed schema plan generated
- [ ] `07-database-agent` (schema plan approved by user at 3c-APPROVE gate)
- [ ] `07-database-agent` (run-migration) confirmed migration completed and `.gstack/impl-database.md` written
...
- [ ] `output/changelog.md` written (STEP 6)
- [ ] `output/agent-execution-log.md` written (STEP 6b)
...
### 7a — Delegate to `13-github-agent`: create-feature-branch
Delegate to `13-github-agent` with operation `create-feature-branch`:
- `owner`, `repo`
- `base-branch`: `<base-branch>` (the branch user selected in incremental mode, or `main` for greenfield)
- `repo-mode`: `<repo-mode>` (from context.md: `incremental` or `greenfield`)
- `work-item-id`: `<work-item-id>` (used for greenfield to create `feature/<work-item-id>-v1`)

This operation will create the appropriate versioned branch:

**If incremental:**
- Parse version from user-selected branch (e.g., `feature/SCRUM-18-v1` → v1)
- Auto-increment it (v1 → v2)
- Create: `feature/SCRUM-18-v2` ✓

**If greenfield:**
- Create feature branch from work-item-id: `feature/<work-item-id>-v1`
- Example: `feature/SCRUM-18-v1` ✓

The operation will then:
1. Create the new branch on GitHub
2. Verify it exists
3. Return the `feature-branch` name

Store the returned `feature-branch` value for use in 7b.

### 7b — Delegate to `13-github-agent`: list-files-for-approval
Delegate to `13-github-agent` with operation `list-files-for-approval`:
- `owner`, `repo`
- `feature-branch`: `<feature-branch>` (from 7a)
- `local-workspace-root`: `<local-workspace-root>`
- `project-root`: `<project-root>`

This operation will:
1. Detect all changed and new files using `git diff`
2. Filter out `.gstack/`, `output/`, `node_modules/`, etc.
3. **Present the file list to user**
4. **Ask for approval: "Do you approve pushing these files to <feature-branch>?"**
5. **Wait for user response (yes/no/list)**
6. Return `approval-status` to orchestrator

**HARD STOP HERE** — The agent will wait for the user's approval response. Once received, proceed to 7c.

### 7c — Delegate to `13-github-agent`: commit-files
Delegate to `13-github-agent` with operation `commit-files`:
- `owner`, `repo`
- `feature-branch`: `<feature-branch>` (from 7a)
- `approval-status`: `<approval-status>` (from 7b — must be "approved")
- `commit-message`: `feat(<work-item-id>): <story-title>`
- `local-workspace-root`: `<local-workspace-root>`
- `project-root`: `<project-root>`

**Safety Gate:** If `approval-status` ≠ "approved", this operation will HALT and report "Commit cancelled — user did not approve."

If approved, the agent will use `git diff` to detect only changed and new files — it does NOT commit the full repo. All commits go to `<feature-branch>`.

### 7d — Delegate to `13-github-agent`: create-pr
Delegate to `13-github-agent` with operation `create-pr`:
- `owner`, `repo`, `work-item-id`, `story-title`
- `feature-branch`: `<feature-branch>` (from 7a)
- `base-branch`: `main` (the PR target)
- `pr-body`: contents of `output/changelog.md`
