...
**Before returning to orchestrator**, append your usage log entry to `.gstack/usage-log.jsonl`:
...
args: {
  ...
  "content": "{\"agent\":\"05-frontend-writer\",\"phase\":\"Frontend\",\"status\":\"<success|failed|skipped>\",\"ai_calls\":<count of LLM calls>,\"input_tokens\":<total>,\"output_tokens\":<total>,\"model\":\"<model-name>\",\"mcp_calls\":<count>,\"start_time\":\"<iso-start>\",\"end_time\":\"<iso-now>\",\"duration_ms\":<elapsed>,\"notes\":\"<error or empty>\"}\n",
  "appendMode": true
}
```

**Fields you provide:**
- `ai_calls`: Number of LLM API calls made (e.g., 1 for single call, 2 for multiple)
- `input_tokens`: Total tokens sent to LLM (estimate OK)
- `output_tokens`: Total tokens received from LLM (estimate OK)
- `model`: Model used, e.g., `"claude-haiku-4.5"` or `"claude-sonnet-4.5"`

**DO NOT include:**
- ❌ `model_multiplier` — System auto-looks this up from model field
- ❌ `premium_requests` — System auto-calculates this

**See USAGE_LOG_FORMAT.md for full format specification.**
